...
const PromoEditScreen = () => {
    ...
    // Function to toggle edit mode and set promo details for editing
    const toggleEditMode = (id) => {
        // Find the promo code being edited
        const promoToEdit = promos.find(promo => promo.id === id);
        // Set the existing data of the promo code in the form fields
        setNewPromo({ ...promoToEdit });
        // Set the edit mode status
        setEditMode({ id: id, status: !editMode.status });
    };

    // Function to handle saving edits
    const saveEdit = async (promoId) => {
        try {
            // Send a PUT request to update the promo code
            await axios.put(`/api/promos/${promoId}/`, newPromo);
            // Refresh the promo list after saving edits
            fetchPromos();
            // Exit edit mode
                    ...
                    <div>
                        {editMode.id === promo.id ? (
                            <div>
                                ...
                                <button className="btn btn-primary" onClick={() => saveEdit(promo.id)}>Save</button>
                            </div>
                        ) : (
                            <div>
                                <button className="btn btn-primary" style={{ marginRight: '.5rem' }} onClick={() => toggleEditMode(promo.id)}>Edit</button>
